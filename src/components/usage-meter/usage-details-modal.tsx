...
function ProviderLineRenderer({ line }: { line: UsageLine }) {
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-medium text-primary-900">
            ...
            {line.limit && line.format === 'dollars' ? ` / $${line.limit.toFixed(2)}` : ''}
        ...
        {line.resetsAt ? (
          <div className="text-[10px] text-primary-400">{formatResetTime(line.resetsAt)}</div>
        ) : null}
      </div>
    )
  }
  ...
  return (
    <div className="flex items-center justify-between text-xs">
      <span className="text-primary-600">{line.label}</span>
      <span className="font-medium text-primary-900">{formatLineValue(line)}</span>
    </div>
  )
}
...
function statusBadge(status: ProviderUsage['status']) {
  switch (status) {
    case 'ok':
      return <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-medium text-emerald-700">Connected</span>
    case 'auth_expired':
      return <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700">Auth Expired</span>
    case 'missing_credentials':
      return <span className="rounded-full bg-neutral-100 px-2 py-0.5 text-[10px] font-medium text-neutral-600">Not Configured</span>
    case 'error':
...
function buildCsv(usage: UsageSummary): string {
  ...
  usage.models.forEach((model) => {
    rows.push(`${model.model},${model.inputTokens},${model.outputTokens},${model.costUsd.toFixed(4)}`)
  })
  rows.push('')
  ...
  usage.sessions.forEach((session) => {
    rows.push(`${session.id},${session.model},${session.inputTokens},${session.outputTokens},${session.costUsd.toFixed(4)},${formatTimestamp(session.startedAt)},${formatTimestamp(session.updatedAt)}`)
  })
  return rows.join('\n')
}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-primary-200 bg-white/60 p-3">
              <div className="text-xs uppercase tracking-wide text-primary-500">Input Tokens</div>
              <div className="text-xl font-semibold text-primary-900">{formatTokens(usage.inputTokens)}</div>
            </div>
            <div className="rounded-2xl border border-primary-200 bg-white/60 p-3">
              <div className="text-xs uppercase tracking-wide text-primary-500">Output Tokens</div>
              <div className="text-xl font-semibold text-primary-900">{formatTokens(usage.outputTokens)}</div>
            </div>
            <div className="rounded-2xl border border-primary-200 bg-white/60 p-3">
              <div className="text-xs uppercase tracking-wide text-primary-500">Daily Cost</div>
              <div className="text-xl font-semibold text-primary-900">{formatCurrency(usage.dailyCost)}</div>
            </div>
          </div>

          <div className="rounded-2xl border border-primary-200 bg-white/70 p-4">
            <div className="mb-3 text-sm font-semibold text-primary-900">Cost per model</div>
            <div className="grid gap-2">
              {usage.models.length === 0 ? (
                <div className="text-sm text-primary-500">No model usage reported yet.</div>
              ) : (
                usage.models.map((model) => (
                  <div key={model.model} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-primary-100 bg-white px-3 py-2 text-sm">
                    <div className="font-medium text-primary-800">{model.model}</div>
                    <div className="text-primary-600">{formatTokens(model.inputTokens)} in · {formatTokens(model.outputTokens)} out</div>
                    <div className="font-semibold text-primary-900">{formatCurrency(model.costUsd)}</div>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="rounded-2xl border border-primary-200 bg-white/70 p-4">
            <div className="mb-3 text-sm font-semibold text-primary-900">Session history</div>
            <div className="grid gap-2">
              {usage.sessions.length === 0 ? (
                <div className="text-sm text-primary-500">No sessions reported yet.</div>
              ) : (
                usage.sessions.map((session) => (
                  <div key={session.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-primary-100 bg-white px-3 py-2 text-sm">
                    <div>
                      <div className="font-medium text-primary-800">{session.id}</div>
                      <div className="text-xs text-primary-500">{session.model}</div>
                    </div>
                    <div className="text-primary-600">{formatTokens(session.inputTokens)} in · {formatTokens(session.outputTokens)} out</div>
                    <div className="text-xs text-primary-500">{formatTimestamp(session.startedAt)} → {formatTimestamp(session.updatedAt)}</div>
                    <div className="font-semibold text-primary-900">{formatCurrency(session.costUsd)}</div>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="grid gap-3">
            {providerUsage.length === 0 ? (
              <div className="rounded-2xl border border-primary-200 bg-white/70 p-6 text-center">
                <div className="text-sm font-medium text-primary-700">No providers connected</div>
                <div className="mt-1 text-xs text-primary-500">
                  Authenticate with Claude CLI or set API keys for OpenAI/OpenRouter to see usage data.
                </div>
              </div>
            ) : (
